...
#define FFCHARS_SIZE		4096
static u_char ffchars[FFCHARS_SIZE];

/*
 * NAND low-level MTD interface functions
 */
static void nand_write_buf(struct mtd_info *mtd, const u_char *buf, int len, int layout_idx, int index);
static void nand_read_buf(struct mtd_info *mtd, u_char *buf, int len, int layout_idx, int index, int page);
 ...
 */
static void nand_write_buf16(struct mtd_info *mtd, const u_char *buf, int len, int layout_idx, int index)
{
	int i;
	struct nand_chip *this = mtd->priv;
	u16 *p = (u16 *) buf;
 ...
 */
static void nand_read_buf16(struct mtd_info *mtd, u_char *buf, int len, int layout_idx, int index, int page)
{
	int i;
	struct nand_chip *this = mtd->priv;
	u16 *p = (u16 *) buf;
	...
	u_char *oob_buf,  struct nand_oobinfo *oobsel, int cached)
{
	...
	case NAND_ECC_NONE:
		...
		if (!this->layout) {
			this->write_buf(mtd, this->data_poi, mtd->oobblock, 0, 0);
			this->write_buf(mtd, oob_buf, mtd->oobsize, 0, 0);
			break;
		}
		 ...
		 * calculating ECC.
		 */
		for (oobidx = 0, x = 0; eccsteps; eccsteps--, x++) {
			int j = 0;
			for (; this->layout[j].length; j++) {
				...
				case ITEM_TYPE_DATA:
					this->write_buf(mtd, &this->data_poi[datidx], this->layout[j].length, j, x);
					datidx += len;
					break;
				case ITEM_TYPE_ECC:
				case ITEM_TYPE_OOB:
					...
					this->write_buf(mtd, &oob_buf[oidx], len, j, x);
					oobidx += len;
					break;
				}
			}
	...
	default:
		...
		if (! this->layout) {
			for (; eccsteps; eccsteps--) {
				...
				this->write_buf(mtd, &this->data_poi[datidx], this->eccsize, 0, 0);
				this->calculate_ecc(mtd, &this->data_poi[datidx], ecc_code);
				...
				if (this->options & NAND_HWECC_SYNDROME)
					this->write_buf(mtd, ecc_code, eccbytes, 0, 0);
				datidx += this->eccsize;
			}
	...
	u_char *oob_buf, struct nand_oobinfo *oobsel, int chipnr, int oobmode)
{
	...
	int	hweccbytes; 
	u_char 	oobdata[NAND_MAX_OOBSIZE];
	...
	for(;;) {
		...
		if (oobmode) {
			...
			this->read_buf(mtd, oobdata, mtd->oobsize - hweccbytes * eccsteps, 0, 0, 0);
			     ...
			     struct nand_oobinfo *oobsel, int flags)
{
	...
	int i, j, x, col, realpage, page, end, ecc, chipnr, sndcmd = 1;
...
/* XXX U-BOOT XXX */
...
#endif
			if (!this->layout) {
				this->read_buf(mtd, data_poi, end, 0, 0, 0);
				break;
			}
			 ...
			 */
			for (oobidx = 0, datidx = 0, x = 0; eccsteps; eccsteps--, x++) {
				for (j = 0; this->layout[j].length; j++) {
					...
					case ITEM_TYPE_DATA:
						...
						this->read_buf(mtd, &data_poi[datidx], len, j, x, page);
					...
					case ITEM_TYPE_OOB:
						...
						this->read_buf(mtd, &oob_data[oidx], len, j, x, page);
		...
		case NAND_ECC_SOFT:	/* Software ECC 3/256: Read in a page + oob data */
			...
			this->read_buf(mtd, &oob_data[mtd->oobsize - oobreadlen], oobreadlen, 0, 0, 0);
			break;	
		...
		default:
			if (! this->layout) {
				for (i = 0, datidx = 0; eccsteps; eccsteps--, i+=eccbytes, datidx += ecc) {
					...
					this->read_buf(mtd, &data_poi[datidx], ecc, 0, 0, 0);
					...
					if (!compareecc) {
						...
						this->read_buf(mtd, &oob_data[i], eccbytes, 0, 0, 0);
			...
			for (oobidx = 0, datidx = 0, eccidx = 0, x = 0; eccsteps; eccsteps--, x++) {
				...
				for (j = 0; this->layout[j].length; j++) {
					...
					case ITEM_TYPE_DATA:
						DEBUG (MTD_DEBUG_LEVEL3, "%s: reading %d bytes of data\n", __FUNCTION__, this->layout[j].length);
						if (x != 0) this->enable_hwecc(mtd, NAND_ECC_READ);
						this->read_buf(mtd, &data_poi[datidx], len, j, x, page);
						datidx += this->layout[j].length;
						break;
...
static int nand_read_oob (struct mtd_info *mtd, loff_t from, size_t len, size_t * retlen, u_char * buf)
{
	...
	*retlen = 0;
	...
	while (i < len) {
		...
		this->read_buf(mtd, &buf[i], thislen, 0, 0, 0);
		i += thislen;
...
/**
 * nand_davinci_read_oob_hwecc - [MTD Interface] NAND read out-of-band (HW ECC)
 * @mtd:	MTD device structure
 * @from:	offset to read from
 * @len:	number of bytes to read
 * @retlen:	pointer to variable to store the number of read bytes
 ...
 */
static int nand_davinci_read_oob_hwecc (struct mtd_info *mtd, loff_t from, size_t len, size_t * retlen, u_char * oob_buf)
{
	int i, j, col, page, chipnr, nleft;
	struct nand_chip *this = mtd->priv;
	char *toobbuf;

	/* Allocate a temporary buffer to hold the oob data to be passed to
	 * the write_buf function.
	 */
	toobbuf = kmalloc(mtd->oobsize, GFP_KERNEL);
	...
	*retlen = 0;
	...
	nleft = len;
	while (i < len) {
		this->cmdfunc(mtd, NAND_CMD_READ0, mtd->oobblock,
				page & this->pagemask);
		for (j = 0; j < mtd->oobsize; j++)
			toobbuf[j] = readb(this->IO_ADDR_R);

		if (mtd->oobsize <= len) {
			memcpy(&oob_buf[i], toobbuf, mtd->oobsize);
		} else {
			memcpy(&oob_buf[i], toobbuf, nleft);
		}
		i += mtd->oobsize;
			...
			for (j = 0; this->layout[j].length; j++) {
				...
				case ITEM_TYPE_OOB:
					...
					if (last - first > 1) {
						...
						this->read_buf(mtd, oob_buf + i, n, 0, 0, 0);
						i += n;
						first += n;
					}
...
int nand_read_raw (struct mtd_info *mtd, uint8_t *buf, loff_t from, size_t len, size_t ooblen)
{
	...
	while (len) {
		...
		if (!this->layout)
			this->read_buf(mtd, &buf[cnt], pagesize, 0, 0, 0);
		else {
...
static int nand_write_oob (struct mtd_info *mtd, loff_t to, size_t len, size_t * retlen, const u_char * buf)
{
	...
	*retlen = 0;
	...
	if (NAND_MUST_PAD(this)) {
		...
		this->write_buf(mtd, ffchars, column, 0, 0);
		/* write data */
		this->write_buf(mtd, buf, len, 0, 0);
		/* postpad 0xff for partial programming */
		this->write_buf(mtd, ffchars, mtd->oobsize - (len+column), 0, 0);
	} else {
		/* Write out desired data */
...
/**
 * nand_davinci_write_oob_hwecc - [MTD Interface] NAND write out-of-band
 * @mtd:	MTD device structure
 * @to:		offset to write to
 * @len:	number of bytes to write
 * @retlen:	pointer to variable to store the number of written bytes
 ...
 * W/o assumptions that are valid only for software ECC
 */
static int nand_davinci_write_oob_hwecc (struct mtd_info *mtd, loff_t to, size_t len, size_t * retlen, const u_char * oob_buf)
{
	...
	*retlen = 0;
	...
	for (ooblen = 0, oc = 0, x = 0; eccsteps; eccsteps--, x++) {
		int j, first, last, thislen;
		/*
		 * In the following we assume that each item (data, ECC,
		...
		for (j = 0; this->layout[j].length; j++) {
			...
			switch (this->layout[j].type) {
			case ITEM_TYPE_DATA:
				this->write_buf(mtd, ffchars, thislen, j, x);
				continue;
			case ITEM_TYPE_ECC:
			case ITEM_TYPE_OOB:
				/*
				...
				if (first >= last) {
					/* no intersection */
					memcpy(&toobbuf[offset], ffchars, thislen);
					offset += thislen;
					break;
				}
				/* pre-pad */
				if (first > ooblen + 1) {
					...
					memcpy(&toobbuf[offset], ffchars,
						((first - ooblen) & ~1));
					offset+= ((first - ooblen) & ~1);
				}
				/* handle an odd offset */
				if (first & 1) {
					__u16 temp = cpu_to_le16((oob_buf[oc++] << 8) | 0xff);
					memcpy(&toobbuf[offset], &temp, 2);
					offset += 2;
					++first;
				}
				if (last - first > 1) {
					...
					memcpy(&toobbuf[offset], oob_buf + oc, n);
					offset += n;
					oc += n;
					first += n;
				}
				/* handle an odd length */
				if (last - first == 1) {
					__u16 temp = cpu_to_le16(0xff00 | oob_buf[oc++]);
					memcpy(&toobbuf[offset], &temp, 2);
					offset += 2;
					++first;
				}
				...
				if (((last + 1) & ~1) < ooblen + thislen) {
					memcpy(&toobbuf[offset], ffchars,
						ooblen + thislen
						- ((last + 1) & ~1));
					offset += ooblen + thislen - ((last + 1) & ~1);
				}
				this->write_buf(mtd, &toobbuf[offset - thislen], thislen, j, x);
				break;
			}
			ooblen += thislen;
		}
	...
	*retlen = 0;
	...
	for (ooblen = 0, oc = 0; eccsteps; eccsteps--) {
		...
		for (j = 0; this->layout[j].length; j++) {
			...
			switch (this->layout[j].type) {
			case ITEM_TYPE_DATA:
				this->write_buf(mtd, ffchars, thislen, 0, 0);
				continue;
			case ITEM_TYPE_ECC:
			case ITEM_TYPE_OOB:
				/*
				...
				if (last - first > 1) {
					int n = ((last - first) & ~1);
					/* write an even number of oob bytes */
					this->write_buf(mtd, oob_buf + oc, n, 0, 0);
					oc += n;
					first += n;
				}
				/* handle an odd length */
...
/* XXX U-BOOT XXX */
	...
	if ((this->eccmode != NAND_ECC_NONE && this->eccmode != NAND_ECC_SOFT)
	    && this->layout) {
#ifdef CFG_DM355_NAND_STD_LAYOUT
		mtd->read_oob = nand_davinci_read_oob_hwecc;
		mtd->write_oob = nand_davinci_write_oob_hwecc;
#else
